...
function create_child_dict(){
    ...
    for(var i = 0; i < all_data.length; i++){
        //1列目の値が10以下の場合、辞書に追加
        if(all_data[i][0] >= 1 && all_data[i][0] <= 27){
            var dict = {};
            var key = "詳細ID";
...
function parent_child_dict(){
    ...
    for(var i = 0; i < parent_data.length; i++){
        var key = parent_data[i][0]; // A列をキーにする
        var value = parent_data[i][1]; // B列目を値にする
        parent_dict[key] = value;
    }
...
function outputToTestTab(){
    ...
    var test_sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("テスト用");
